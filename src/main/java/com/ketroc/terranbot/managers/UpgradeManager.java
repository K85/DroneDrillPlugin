...
public class UpgradeManager {
    public static final List<Upgrades> shipAttack;
    public static final List<Upgrades> shipArmor;

    static {
        shipAttack = new ArrayList(List.of(Upgrades.TERRAN_SHIP_WEAPONS_LEVEL1, Upgrades.TERRAN_SHIP_WEAPONS_LEVEL2, Upgrades.TERRAN_SHIP_WEAPONS_LEVEL3));
        shipArmor = new ArrayList(List.of(Upgrades.TERRAN_VEHICLE_AND_SHIP_ARMORS_LEVEL1, Upgrades.TERRAN_VEHICLE_AND_SHIP_ARMORS_LEVEL2, Upgrades.TERRAN_VEHICLE_AND_SHIP_ARMORS_LEVEL3));
    }
    ...
    public static void updateUpgradeList() {
        Bot.OBS.getUpgrades().stream().forEach((upgrade) -> {
    ...
    private static void checkArmories() {
        if (!shipArmor.isEmpty() || !shipAttack.isEmpty()) {
            ...
            Unit idleArmory = armories.stream().filter((unit) -> {
    ...
    private static Upgrades getNextArmoryUpgrade(List<Unit> armories) {
        Optional<Unit> activeArmory = armories.stream().filter((unit) -> {
            return !unit.getOrders().isEmpty();
        }).findFirst();
